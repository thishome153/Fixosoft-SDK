...
void CMyStatusBar::OnAttach() 
{
	...
	SendMessage(SB_SETBKCOLOR, 0, RGB(125, 224, 0));
   ...
   //SetPartIcon(2, (HICON)GetApp().LoadImage(MAKEINTRESOURCE(IDW_MAIN), IMAGE_ICON, 16, 16));
	...
	//	SetPartText(3, _T(""), SBT_OWNERDRAW);
      ...
      // SetPartText(0, _T("StatusBar[0]"), SBT_OWNERDRAW);
	...
	case SB_SETTEXT:
		{
		  if ((int) wParam == 2) 
		 {
		   char* text1 = "wndProc _SB_TEXT";
		    this->SetPartText(2, (LPCTSTR) text1);//(LPCTSTR) lParam , 0);
